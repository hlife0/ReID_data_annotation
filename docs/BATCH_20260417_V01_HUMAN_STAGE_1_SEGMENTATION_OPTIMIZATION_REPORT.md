...
如果只改一件事，把这组对照配置从 `FP5 + SP3` 改成 `FP6 + SP3`：
...
## 9. 本次 sweep 中较平衡的配置候选

综合本次 sweep 的结果，一组较平衡的候选配置是：
