...
class HumanStage1AdminWebStaticTests(unittest.TestCase):
    def test_human_stage_1_admin_ui_exposes_stage1_specific_sections(self) -> None:
        ...
        self.assertIn('fetchJson("/api/overview")', js)
        self.assertIn('fetchJson("/api/annotators")', js)
        self.assertIn('fetchJson("/api/segments")', js)
        self.assertIn("/api/segment_detail?segment_id=", js)
